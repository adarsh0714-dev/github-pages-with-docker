...
(function() {
      ...
      if (results.length) { // Are there any results?
          ...
          for (var i = 0; i < results.length; i++) { // Iterate over the results
              ...
              if (item) { // Ensure item exists
                  appendString += '<li><a href="' + item.url + '"><h3>' + item.title + '</h3></a>';
                  appendString += '<p>' + (item.content.length > 150 ? item.content.substring(0, 150) + '...' : item.content) + '</p></li>';
              }
          }
  ...
  if (searchTerm) {
      ...
      if (typeof window.store === 'undefined') {
          console.error('Window store is not defined.');
          return;
      }
      ...
      var results = idx.search(searchTerm); // Get lunr to perform a search
      displaySearchResults(results, window.store); // Display the search results
  }
})();
